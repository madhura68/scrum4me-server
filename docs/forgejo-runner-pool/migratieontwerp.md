...
Een **bevestigde harde trustafwijking** is: een onbekende of niet-goedgekeurde workflow-schrijver, een niet-bevestigd fork/PR-pad waarmee onbetrouwbare code op de gedeelde labels kan starten (zie de acknowledged-uitzondering hieronder), of een na geslaagde bron-readiness inhoudelijk niet-uitleesbare relevante instelling. Alleen dan gaan de cyclecontrollers op beide hosts naar `QUARANTINED`, accepteert de pool geen nieuwe jobs en is global scope NO-GO; als het niet veilig kan worden hersteld, worden de runners beperkt tot organisatie- of repositoryscope voor uitsluitend goedgekeurde repositories.

**Bevestigde risicovolle triggers (`risky_triggers_acknowledged`) — ISS-9-delta, 2026-09-05.** Een repository mag in `trusted-actions-scope.yml` een per-repo lijst `risky_triggers_acknowledged` dragen: risicovolle triggers (`pull_request`, `pull_request_target`, `workflow_run`) waarvan JP de kruising met een gedeeld runnerlabel expliciet aanvaardt. `classify()` merkt zo'n bevestigde kruising aan als **`accepted`** in plaats van `hard`; uitsluitend die trigger voor die repo wordt verzacht, en iedere andere harde afwijking (onbekende schrijver, Actions-drift) blijft hard. Alleen een lijst waarvan elk lid een bekende trigger is telt als bevestiging: iedere andere vorm — een string (ook een gequote `"[...]"`), een dict, of een lijst met een onbekend of niet-string lid — is fail-closed géén bevestiging en laat de kruising hard, met een zachte melding zodat de misconfiguratie zichtbaar is.

**Deze bevestiging is een bewuste risico-acceptatie door JP, geen gevolgtrekking uit een instance-setting.** Twee gemeten feiten begrenzen de blootstelling, maar bewijzen het besluit niet en worden hier daarom apart gehouden van de trustbeslissing:

- `[service] DISABLE_REGISTRATION = true` (gemeten 2026-09-05) sluit nieuwe zelfregistratie. Dat maakt de account-roster gesloten, maar bewijst niet dat iedere reeds bestaande account vertrouwd is of door JP bij naam voor deze runnertrust is beoordeeld. §7.7 stelt zelf dat een identiteit alleen trusted is als JP haar bij naam heeft goedgekeurd; een fork-PR kan echter van elk bestaand account komen, niet alleen van de bij naam goedgekeurde schrijvers van de doelrepo. De trustgate inventariseert de schrijvers van de doelrepo, niet de volledige set accounts die een fork-PR kan aanbieden.
- Forgejo 15.0.2 kent een **per-repository** fork-PR-approvalgate (`RequireApprovalForForkPRWorkflows`, per run zichtbaar als `need_approval`/`approved_by`/`is_fork_pull_request`); de eerste fork-PR van een auteur vereist goedkeuring. De effectieve instelling per bevestigde repo is hier niet vastgelegd en wordt door de gate niet getoetst.

Wat JP met een bevestiging aanvaardt, is dus dat het fork/PR-pad voor die specifieke repo acceptabel is op de gedeelde privileged runner, gegeven de gesloten registratie en de approvalgate die hij zelf beheert — het is een risico-acceptatie, geen bewezen afwezigheid van risico. Het **aanvaarde restrisico** is expliciet: een bestaand (door JP aangemaakt) account kan na een eenmalig goedgekeurde fork-PR daarna automatisch nieuwe of gewijzigde workflows op de privileged DinD draaien, en een containerescape daaruit raakt host en productie (§7.6). Fork-PR-workflows krijgen geen repo-secrets, maar dat dekt de escape niet. JP draagt daarom de verantwoordelijkheid de eerste fork-PR-approval per bevestigde repo bewust te verlenen. Een wijziging in de account-roster of in de effectieve approvalpolicy van een bevestigde repo vraagt om herbeoordeling van de betreffende bevestiging; een bevestiging is per repo intrekbaar door de regel te verwijderen, waarna de kruising weer `hard` is. De eerder in ISS-9 veronderstelde globale setting `DEFAULT_ALLOW_ONLY_CONTRIBUTORS_TO_TRIGGER_ACTIONS` bestaat **niet** in Forgejo 15.0.2 (gemeten tegen binary en upstream-bron op tag v15.0.2) en is bewust niet de grond voor deze uitzondering.
...
### Delta-review R13 — caps-methodiek — GESLOTEN en teruggedraaid; het ontwerp blijft ongewijzigd — 1 september 2026
...
**Besluit (JP, 1 september 2026):** het ontwerp blijft **ongewijzigd** en de uitvoering gaat door. De caps blijven zoals §7.8 ze voorschrijft en zoals `evidence/stap-a/caps.env` ze vastlegt: runner 1,0 vCPU / 1 GiB / 256 PID, DinD 11,5 vCPU / 5 GiB / 2048 PID. De caps-methodiek-delta is daarmee **overbodig en teruggedraaid**; §7.8 staat weer in de GO'de R12-staat. De bestaande runner op `scrum4me-server` blijft ongewijzigd draaien tot stap G, precies zoals §7.2 al voorschrijft; §7.3, stap G en §12 blijven onverkort van kracht. Twee onderweg overwogen alternatieven zijn verworpen en worden hier vastgelegd zodat ze niet terugkeren als open vraag: de pool alleen op `max2` draaien met `scrum4me-server` afgevoerd als runnerhost, en de legacy runner uitfaseren of op een eigen label zetten. Er volgt dus **geen** zelfstandige ontwerprevisie.

**Waarom de ROOD gemeten headroomgate de uitvoering niet blokkeert:** §7.8 is een **pre-mutatie**-gate — "`preflight.sh` faalt vóór iedere mutatie als op de betrokken host niet geldt". Stap C tot en met F muteren uitsluitend `max2`, en daar is de gate GROEN (`evidence/stap-a/preflight-max2.txt`, exit 0): CPU-som 12,5 ≤ 14 vCPU en geheugensom 6,0 ≤ 9,79 GiB. Op `scrum4me-server` wordt in die stappen niets gemuteerd, dus gate't de gate daar niets. Aanvullende grond (JP): de runner draait op die host al maanden **ongecapt** naast productie — dus zonder enige bovengrens — en dat heeft zich gehouden.
...
**Besluit stap G — GENOMEN (JP, 8 september 2026; record-and-proceed, eigen verantwoordelijkheid).** De §7.8-headroomgate wordt voor `scrum4me-server` bewust **gewaiverd**. Grond: de runner draait daar aantoonbaar al maanden **ongecapt** naast productie zonder incident — sterker bewijs dan een synthetische 2×-headroomdrempel; de formele headroom-*proof* (preflight) is de werkelijke struikelblok, niet de capaciteit, en die eis vervalt daarom voor deze host. De waiver is **scoped tot `scrum4me-server`**: op `max2` blijft de gate onverkort gelden (daar is hij groen). De runner-**caps blijven** van kracht (identiek op beide hosts) — strikt veiliger dan de huidige ongecapte legacy-runner: de DinD-CPU-cap wordt als `cpu_period`/`cpu_quota` uitgedrukt (host-portabel; de oorspronkelijke `cpus:`/NanoCpus-vorm werd door Docker geweigerd bij create op een 8-core host — 11,5 > 8 — en is daarom in een aparte gedeelde bundelwijziging vervangen) en bindt op 8 cores niet (11,5-quota > 8 fysieke cores), en de 5 GiB DinD-geheugencap voegt een plafond toe dat de ongecapte legacy mist. Compenserende controls: `capacity: 1`, CPU-prioriteit voor productie, monitoring met auto-pause (§10) bij productie-OOM/throttle of lage `MemAvailable`, en pool-redundantie (`max2` primair; deze runner is sinds stap D/E redundant). **`preflight.sh` wordt NIET versoepeld**: hij rapporteert op deze host terecht ROOD (exit 40); de beslissing om toch door te gaan is déze gedocumenteerde waiver, niet een groene gate. Hiermee vervalt de hardware-eis (optie "host vergroten") en draait stap G op de huidige box. JP heeft afgezien van een delta-review op deze waiver. Uitwerking: `implementatieplan-stap-g.md` (Voorwaarde 0).
