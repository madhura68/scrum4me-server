...
**Bevestigde risicovolle triggers (`risky_triggers_acknowledged`) — ISS-9-delta, 2026-09-05.** Een repository mag in `trusted-actions-scope.yml` een per-repo lijst `risky_triggers_acknowledged` dragen: risicovolle triggers (`pull_request`, `pull_request_target`, `workflow_run`) waarvan JP de kruising met een gedeeld runnerlabel expliciet aanvaardt. `classify()` merkt zo'n bevestigde kruising aan als **`accepted`** in plaats van `hard`; uitsluitend die specifieke trigger voor die specifieke repo wordt verzacht, iedere andere harde afwijking (onbekende schrijver, Actions-drift) blijft hard, en een verkeerd getypt veld telt fail-closed als geen bevestiging. Dit operationaliseert de "vertrouwde gebruikers"-clausule uit §7.6 en berust op twee **gemeten** instance-eigenschappen, niet op een vermoeden. Ten eerste staat op de instance `[service] DISABLE_REGISTRATION = true` (gemeten 2026-09-05): er bestaan geen self-service accounts, dus een fork-PR kan alleen van een door JP aangemaakt, vertrouwd account komen. Ten tweede kent Forgejo 15.0.2 een **per-repository** fork-PR-approvalgate (`RequireApprovalForForkPRWorkflows`, per run zichtbaar als `need_approval`/`approved_by`/`is_fork_pull_request`); de eerste fork-PR van een auteur vereist goedkeuring. Het **aanvaarde restrisico** is expliciet: een vertrouwd account kan na een eenmalig goedgekeurde fork-PR daarna automatisch nieuwe of gewijzigde workflows op de privileged DinD draaien, en een containerescape daaruit raakt host en productie (§7.6). Fork-PR-workflows krijgen geen repo-secrets, maar dat dekt de escape niet. JP draagt daarom de verantwoordelijkheid de eerste fork-PR-approval per bevestigde repo bewust te verlenen; een bevestiging is per repo intrekbaar door de regel te verwijderen, waarna de kruising weer `hard` is. De eerder in ISS-9 veronderstelde globale setting `DEFAULT_ALLOW_ONLY_CONTRIBUTORS_TO_TRIGGER_ACTIONS` bestaat **niet** in Forgejo 15.0.2 (gemeten tegen binary en upstream-bron op tag v15.0.2) en is bewust niet de grond voor deze uitzondering.
